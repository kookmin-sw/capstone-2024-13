...
if [ "$originalArgOne" = 'mongod' ]; then
	...
	if [ -n "$shouldPerformInitdb" ]; then
		...
		_mongod_hack_ensure_arg_val --bind_ip "$MONGO_INITDB_BIND_IP" "${mongodHackedArgs[@]}"
		_mongod_hack_ensure_arg_val --port "$MONGO_INITDB_PORT" "${mongodHackedArgs[@]}"
		...
		mongo=( "$mongoShell" --host "$MONGO_INITDB_BIND_IP" --port "$MONGO_INITDB_PORT" --quiet )
